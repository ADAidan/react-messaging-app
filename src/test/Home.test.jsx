...
describe("Home", () => {
  it("should render the Welcome to Concord header", () => {
    ...
    const headerElement = screen.getByRole("heading", {
      name: /Welcome to Concord/i,
    });
    expect(headerElement).toBeInTheDocument();

    const joinForFreeButton = screen.getByRole("link", {
    ...
    const connectWithOldFriendsText = screen.getByText(
      /Connect with old friends or meet new ones/i,
    );
    expect(connectWithOldFriendsText).toBeInTheDocument();

    const createOrJoinCommunityText = screen.getByText(
      /Create or join a thriving community/i,
    );
